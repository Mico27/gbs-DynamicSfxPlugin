...
| Budget | Cost |
|---|---|
| Bank 0 (HOME) | +46 bytes |
| WRAM | +34 bytes |
| Banked ROM | +1,261 bytes |

- **Bank 0:** 46 bytes are resident in the non-switchable bank (`dynamic_sfx_player.c`); everything else lives in a switchable bank. See [Bank 0 (HOME) Usage](#bank-0-home-usage).
- **WRAM:** 34 bytes of synthesis and playback state.
- **Banked ROM:** the 1,261 bytes are the synthesis engine only — every sound you author with the Compile events adds its own data bytes to ROM on top.
- **Engine WRAM headroom:** a stock GB Studio 4.3.0 project leaves about **854 bytes** of WRAM free (usable engine WRAM is 7,776 bytes at 0xC0A0–0xDF00; the stock engine uses 6,922). With this plugin installed roughly **820 bytes** remain. That does not change with the number of global variables your project defines: the script memory array is a fixed 3,584 bytes at stock engine settings (VM_HEAP_SIZE + VM_MAX_CONTEXTS × VM_CONTEXT_STACK_SIZE = 768 + 16 × 64 words).
- **SRAM:** not used.
...
|---|---|---|---|
| `core/dynamic_sfx_player.c` | 46 | — *(new file)* | +46 |
